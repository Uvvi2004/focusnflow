...
// Study Groups screen — lists all groups split into My Groups and All Groups.
// Members can join, leave, schedule a session, start a Pomodoro, or delete (creators only).
// All changes reflect in real time for every user through Firestore streams.
...
class _GroupsScreenState extends State<GroupsScreen> {
  ...
  // Create group sheet — creator is automatically the first member
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _SheetTextField(controller: nameController, label: 'Group Name (e.g. CS450 Study Buddies)'),
            const SizedBox(height: 12),
            _SheetTextField(controller: courseController, label: 'Course Tag (e.g. CS450)'),
            const SizedBox(height: 12),
            _SheetTextField(controller: descController, label: 'Description (e.g. Weekly exam prep)'),
            const SizedBox(height: 20),
            ElevatedButton(
              onPressed: () async {
  ...
  // Group detail sheet — action buttons shown depend on the user's role:
  // non-member: Join | member (not creator): Leave | creator: Set Session + Delete
  // Any member can start the Pomodoro timer
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(group.courseTag,
                      style: const TextStyle(
                          color: _accentColor, fontSize: 12, fontWeight: FontWeight.bold)),
                ),
              ],
            ),
            const SizedBox(height: 12),
            ...
            Row(
              children: [
                const Icon(Icons.people_rounded, color: _accentColor, size: 18),
            ...
            if (!isMember)
              ElevatedButton(
                onPressed: () async {
                  try {
                    await _firestoreService.joinGroup(group.groupId, _user.uid);
                    if (context.mounted) Navigator.pop(context);
                  } catch (e) {
                    if (context.mounted) {
            ...
            if (isMember && !isCreator)
              ElevatedButton(
                onPressed: () async {
                  try {
                    await _firestoreService.leaveGroup(group.groupId, _user.uid);
                    if (context.mounted) Navigator.pop(context);
                  } catch (e) {
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
            ...
            if (isCreator) ...[
              ElevatedButton(
                onPressed: () async {
                  ...
                  if (picked != null) {
                    await _firestoreService.updateNextSession(group.groupId, picked);
                    ...
                    // Also send the 24-hour reminder immediately if the session is within 25 hours
                    final hoursUntil = picked.difference(DateTime.now()).inHours;
                ...
                child: const Text('Set Next Session', style: TextStyle(color: Colors.white)),
              ),
              const SizedBox(height: 8),
              ElevatedButton(
                onPressed: () async {
      ...
      body: SafeArea(
        child: Column(
          ...
          children: [
            const Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 4),
                  Text('Join or create a group for your course',
            ...
            // Streams all groups and splits them client-side into
            // My Groups (member) and All Groups (not a member)
            Expanded(
              child: StreamBuilder<List<GroupModel>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (!snapshot.hasData || snapshot.data!.isEmpty) {
                    return Center(
                      child: Column(
                        ...
                        children: [
                          Icon(Icons.group_rounded, color: _subtextColor, size: 48),
                          const SizedBox(height: 16),
                          const Text('No groups yet', style: TextStyle(color: _subtextColor)),
                          const SizedBox(height: 8),
                  ...
                  final myGroups = groups.where((g) => g.members.contains(_user.uid)).toList();
                  final otherGroups = groups.where((g) => !g.members.contains(_user.uid)).toList();
                    ...
                    children: [
                      if (myGroups.isNotEmpty) ...[
                        ...
                        ...myGroups.map((g) => Padding(
                              ...
                              child: _GroupCard(
                                  group: g, userId: _user.uid, onTap: () => _showGroupDetail(g)),
                            )),
                        const SizedBox(height: 16),
                      ],
...
// Group card — accent border when the user is already a member.
// Shows name, member count, course tag, and next session date if set.
class _GroupCard extends StatelessWidget {
  final GroupModel group;
  final String userId;
  final VoidCallback onTap;

  const _GroupCard({required this.group, required this.userId, required this.onTap});
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isMember ? _accentColor.withValues(alpha: 0.3) : Colors.white10,
          ),
        ),
        child: Row(
          children: [
            Container(
              ...
              child: const Icon(Icons.group_rounded, color: _accentColor, size: 22),
            ),
